...
tile(Monitor *m)
{
	...
	int wx = m->wx + tilemargin;
	int wy = m->wy + tilemargin;
	int ww = m->ww - 2 * tilemargin;
	int wh = m->wh - 2 * tilemargin;
	...
	if (n > m->nmaster)
		mw = m->nmaster ? (ww - (g = gappx)) * m->mfact : 0;
	else
		mw = ww;
	for (i = my = ty = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			...
			r = n - i;
			h = (wh - ty - gappx * (r - 1)) / r;
			resize(c, wx + mw + g, wy + ty, ww - mw - g - (2*c->bw), h - (2*c->bw), False);
			ty += HEIGHT(c) + gappx;
		}
}
